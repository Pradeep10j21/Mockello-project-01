...
const CollegeSidebar = ({ onLogout }: CollegeSidebarProps) => {
  const location = useLocation();
  const [collapsed, setCollapsed] = useState(false);

  const menuItems = [
    { icon: LayoutDashboard, label: "Dashboard", path: "/college/dashboard" },
    { icon: UserCircle, label: "College Profile", path: "/college/profile" },
    { icon: Building2, label: "Companies", path: "/college/companies" },
    { icon: Target, label: "Eligibility Tracker", path: "/college/eligibility" },
  ];
    ...
    >
      ...
      <div className="p-4 border-b border-sidebar-border">
        <Link to="/college/dashboard" className="flex items-center gap-3">
          ...
          {!collapsed && (
            <span className="font-display text-lg font-semibold">GreenPath</span>
          )}
        </Link>
      </div>
      ...
      <button
        ...
        className="absolute -right-3 top-20 w-6 h-6 rounded-full bg-sage text-forest-deep flex items-center justify-center shadow-md hover:bg-sage-light transition-colors"
      >
            ...
            <li key={item.path}>
              <Link
                ...
                className={cn(
                  ...
                  isActive(item.path)
                    ? "bg-sidebar-accent text-sidebar-accent-foreground border-l-4 border-sage"
